...
#import "ShiPinDetailNetManager.h"

@implementation ShiPinDetailNetManager

+(id)getShiPinDetailWith:(long)group_id completionHandle:(void (^)(id, NSError *))completionHandle{
    ...
    return [self GET:path parameters:params completionHandler:^(id responseObj, NSError *error) {
        completionHandle([ShiPinDetailModel objectWithKeyValues:responseObj],error);
    }];
}
@end
